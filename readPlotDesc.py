...
def readAndExportPlotDescs(models):
    ...
    for plot in plots:
        ...
        with open("plot_presets/source_desc_values/" + "READABLE_" + plot.name() + ", " + plot.tag() + ".txt", "w") as file:
        ...
        for subplot in plot_children:
            ...
            properties, properties_readable = get_node_properties(subplot)
            with open("plot_presets/source_desc_values/" + subplot.name() + ", " + subplot.tag() + ".txt", "w") as file:
                file.write(str(properties))
            with open("plot_presets/source_desc_values/" + "READABLE_" + subplot.name() + ", " + subplot.tag() + ".txt", "w") as file:
                file.write(str(properties_readable))
        messages += "\n\n"
